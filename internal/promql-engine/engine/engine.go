...
func (e *Engine) parseExpr(qs string) (parser.Expr, error) {
	orig := parser.Functions
	parser.Functions = e.functions
	defer func() { parser.Functions = orig }()

	p := e.parser
	if p == nil {
		p = parser.NewParser(parser.Options{})
	}
	return p.ParseExpr(qs)
}
